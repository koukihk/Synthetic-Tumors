...
def save_synthetic_data(dataset):
    for idx, data_sample in enumerate(dataset):
        synthetic_image = data_sample['image']
        synthetic_label = data_sample['label']

        image_affine = data_sample["image_meta_dict"]["affine"][0].numpy()
        label_affine = data_sample["label_meta_dict"]["affine"][0].numpy()

        # 构造NIfTI图像和标签
        image_nifti = nb.Nifti1Image(synthetic_image, affine=image_affine)
        label_nifti = nb.Nifti1Image(synthetic_label, affine=label_affine)

        # Save the images
        root_dir = 'datafolds'
        output_dir = os.path.join(root_dir, 'synt')
        ...
        nb.save(
            nb.Nifti1Image(image_nifti.astype(np.uint8), image_affine),
            os.path.join(output_img_dir, f'{counter}.nii.gz')
        )

        nb.save(
            nb.Nifti1Image(label_nifti.astype(np.uint8), label_affine),
            os.path.join(output_label_dir, f'{counter}.nii.gz')
        )


def show_synthetic_data_origin(dataset):
    for idx, data_sample in enumerate(dataset):
        print(f"Data sample {idx}: {data_sample}")
        print(type(data_sample))
        if isinstance(data_sample, dict):
            for key, value in data_sample.items():
                print(f"Key: {key}, Value: {value}")

def show_synthetic_data(dataset):
    image_meta_dicts = [data_sample.get('image', {}).get('image_meta_dict', {}) for data_sample in dataset]
    label_meta_dicts = [data_sample.get('label', {}).get('label_meta_dict', {}) for data_sample in dataset]

    root_dir = 'meta_dict'
    existing_files = glob.glob(os.path.join(root_dir, '*.txt'))
    counter = len(existing_files)

    for idx, (image_meta_dict, label_meta_dict) in enumerate(zip(image_meta_dicts, label_meta_dicts)):
        image_meta_str = str(image_meta_dict)
        label_meta_str = str(label_meta_dict)

        with open(f'{root_dir}/{counter + idx}.txt', 'w') as file:
            file.write("Image Meta Data:\n")
            file.write(image_meta_str)
            file.write("\n\nLabel Meta Data:\n")
            file.write(label_meta_str)

        print(f"meta_dict {counter + idx}.txt saved.")
