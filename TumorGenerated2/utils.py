...
def SynthesisTumor(volume_scan, mask_scan, tumor_type, texture):
    ...
    # 裁剪 CT 和 Label，保留感兴趣的区域
    volume_shape = volume_scan.shape
    ...
    # 添加边界
    x_start, x_end = max(0, x_start - 32), min(volume_shape[0], x_end + 32)
    y_start, y_end = max(0, y_start - 32), min(volume_shape[1], y_end + 32)
    z_start, z_end = max(0, z_start - 16), min(volume_shape[2], z_end + 16)

    cropped_volume = volume_scan[x_start:x_end, y_start:y_end, z_start:z_end]
    cropped_mask = mask_scan[x_start:x_end, y_start:y_end, z_start:z_end]

    # 保存裁剪后的 CT 和 Label
    root_dir = 'datafolds'
    output_dir = os.path.join(root_dir, 'synt', tumor_type)
    ...
    nib.save(
        nib.Nifti1Image(cropped_volume.astype(np.uint8), np.eye(4)),
        os.path.join(output_img_dir, f'{counter}.nii.gz')
    )

    nib.save(
        nib.Nifti1Image(cropped_mask.astype(np.uint8), np.eye(4)),
